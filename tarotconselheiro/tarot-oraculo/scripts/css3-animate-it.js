...
$(".animate").appear();
$(".cards-round-deck").appear();
$(".before-results").appear();
$(".associations").appear(); 
$(".animatedParent").appear();
 $(document.body).on('appear', '.associations', function(){
    setTimeout(function () {
        $(".tehConnectingTriangle").addClass("active");
    },22000);
    setTimeout(function () {
        $(".faceGraphic").addClass("active");
    },23200);
}); 
$(document.body).on('appear', '.cards-round-deck', function(){
    $(this).addClass("dealt");
});

/* $(document.body).on('appear', '.before-results', function(){
    var $this = $(this);
    $this.addClass("hexaShape");
    setTimeout(function () {
        $this.find(".card-front-only").addClass("yellow-line");
    },1000);
    setTimeout(function () {
        $this.find(".card-front-only").addClass("showNumbers");
    },2500);
});
*/

$(document.body).on('appear', '.animate', function(){
    if($(window).width() >= 768){
        var ele = $(this);
        ele.addClass('go');
    }
...
$(document.body).on('appear', '.animatedParent', function(e, $affected){
    var ele = $(this).find('.animated-child');
    var parent = $(this);


    if(parent.attr('data-sequence') != undefined){

        var firstId = $(this).find('.animated-child:first').attr('data-id');
        var number = firstId;
        var lastId = $(this).find('.animated-child:last').attr('data-id');

        $(parent).find(".animated-child[data-id="+ number +"]").addClass('go');
        number ++;
